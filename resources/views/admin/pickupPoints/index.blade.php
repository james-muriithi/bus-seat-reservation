...
@can('pickup_point_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.pickup-points.create') }}">
                        ...
                        <tr data-entry-id="{{ $pickupPoint->id }}">
                            ...
                            <td>
                                @can('pickup_point_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.pickup-points.show', $pickupPoint->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('pickup_point_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.pickup-points.edit', $pickupPoint->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('pickup_point_delete')
                                    <form action="{{ route('admin.pickup-points.destroy', $pickupPoint->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pickup_point_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.pickup-points.massDestroy') }}",
