...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pickup-points.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="pickup_point">{{ trans('cruds.pickupPoint.fields.pickup_point') }}</label>
                <input class="form-control {{ $errors->has('pickup_point') ? 'is-invalid' : '' }}" type="text" name="pickup_point" id="pickup_point" value="{{ old('pickup_point', '') }}" required>
