...
@can('bus_rating_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.bus-ratings.create') }}">
                ...
                <tbody>
                    @foreach($busRatings as $key => $busRating)
                        <tr data-entry-id="{{ $busRating->id }}">
                            ...
                            <td>
                                @can('bus_rating_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.bus-ratings.show', $busRating->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('bus_rating_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.bus-ratings.edit', $busRating->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('bus_rating_delete')
                                    <form action="{{ route('admin.bus-ratings.destroy', $busRating->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('bus_rating_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.bus-ratings.massDestroy') }}",
