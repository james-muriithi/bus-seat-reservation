...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.routePriceVariation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.route-price-variations.update", [$routePriceVariation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pickup_point') ? 'is-invalid' : '' }}" name="pickup_point_id" id="pickup_point_id" required>
                    @foreach($pickup_points as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pickup_point_id') ? old('pickup_point_id') : $routePriceVariation->pickup_point->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('drop_point') ? 'is-invalid' : '' }}" name="drop_point_id" id="drop_point_id" required>
                    @foreach($drop_points as $id => $entry)
                        <option value="{{ $id }}" {{ (old('drop_point_id') ? old('drop_point_id') : $routePriceVariation->drop_point->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('seat_classes') ? 'is-invalid' : '' }}" name="seat_classes[]" id="seat_classes" multiple>
                    @foreach($seat_classes as $id => $seat_class)
                        <option value="{{ $id }}" {{ (in_array($id, old('seat_classes', [])) || $routePriceVariation->seat_classes->contains($id)) ? 'selected' : '' }}>{{ $seat_class }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fare') ? 'is-invalid' : '' }}" type="number" name="fare" id="fare" value="{{ old('fare', $routePriceVariation->fare) }}" step="0.01">
                @if($errors->has('fare'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fare') }}
                    </div>
