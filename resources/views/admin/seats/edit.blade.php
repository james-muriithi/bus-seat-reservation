    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.seats.update", [$seat->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bus') ? 'is-invalid' : '' }}" name="bus_id" id="bus_id" required>
                    @foreach($buses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bus_id') ? old('bus_id') : $seat->bus->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('bus'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('row') ? 'is-invalid' : '' }}" type="number" name="row" id="row" value="{{ old('row', $seat->row) }}" step="1" required>
                @if($errors->has('row'))
                    <div class="invalid-feedback">
                        {{ $errors->first('row') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('column') ? 'is-invalid' : '' }}" type="number" name="column" id="column" value="{{ old('column', $seat->column) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $seat->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('details') ? 'is-invalid' : '' }}" type="text" name="details" id="details" value="{{ old('details', $seat->details) }}">
