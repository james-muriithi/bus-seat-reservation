...
@can('bus_type_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.bus-types.create') }}">
                ...
                <tbody>
                    @foreach($busTypes as $key => $busType)
                        <tr data-entry-id="{{ $busType->id }}">
                            ...
                            <td>
                                @can('bus_type_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.bus-types.show', $busType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('bus_type_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.bus-types.edit', $busType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('bus_type_delete')
                                    <form action="{{ route('admin.bus-types.destroy', $busType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('bus_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.bus-types.massDestroy') }}",
