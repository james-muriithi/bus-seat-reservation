...
@can('drop_off_point_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.drop-off-points.create') }}">
                        ...
                        <tr data-entry-id="{{ $dropOffPoint->id }}">
                            ...
                            <td>
                                @can('drop_off_point_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.drop-off-points.show', $dropOffPoint->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('drop_off_point_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.drop-off-points.edit', $dropOffPoint->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('drop_off_point_delete')
                                    <form action="{{ route('admin.drop-off-points.destroy', $dropOffPoint->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('drop_off_point_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.drop-off-points.massDestroy') }}",
