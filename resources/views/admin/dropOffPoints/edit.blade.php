...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.dropOffPoint.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.drop-off-points.update", [$dropOffPoint->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="drop_off_point">{{ trans('cruds.dropOffPoint.fields.drop_off_point') }}</label>
                <input class="form-control {{ $errors->has('drop_off_point') ? 'is-invalid' : '' }}" type="text" name="drop_off_point" id="drop_off_point" value="{{ old('drop_off_point', $dropOffPoint->drop_off_point) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('drop_time') ? 'is-invalid' : '' }}" type="text" name="drop_time" id="drop_time" value="{{ old('drop_time', $dropOffPoint->drop_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('landmark') ? 'is-invalid' : '' }}" type="text" name="landmark" id="landmark" value="{{ old('landmark', $dropOffPoint->landmark) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text" name="address" id="address" value="{{ old('address', $dropOffPoint->address) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('route') ? 'is-invalid' : '' }}" name="route_id" id="route_id" required>
                    @foreach($routes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('route_id') ? old('route_id') : $dropOffPoint->route->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('route'))
                    <div class="invalid-feedback">
