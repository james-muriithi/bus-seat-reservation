...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.drop-off-points.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="drop_off_point">{{ trans('cruds.dropOffPoint.fields.drop_off_point') }}</label>
                <input class="form-control {{ $errors->has('drop_off_point') ? 'is-invalid' : '' }}" type="text" name="drop_off_point" id="drop_off_point" value="{{ old('drop_off_point', '') }}" required>
