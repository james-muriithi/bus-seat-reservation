    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.system-settings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('default_currency') ? 'is-invalid' : '' }}" name="default_currency_id" id="default_currency_id" required>
                    @foreach($default_currencies as $id => $entry)
                        <option value="{{ $id }}" {{ old('default_currency_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('smtp_username') ? 'is-invalid' : '' }}" type="text" name="smtp_username" id="smtp_username" value="{{ old('smtp_username', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('smtp_host') ? 'is-invalid' : '' }}" type="text" name="smtp_host" id="smtp_host" value="{{ old('smtp_host', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('smtp_password') ? 'is-invalid' : '' }}" type="text" name="smtp_password" id="smtp_password" value="{{ old('smtp_password', '') }}">
