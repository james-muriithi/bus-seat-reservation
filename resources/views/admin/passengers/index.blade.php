...
@can('passenger_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.passengers.create') }}">
                ...
                <tbody>
                    @foreach($passengers as $key => $passenger)
                        <tr data-entry-id="{{ $passenger->id }}">
                            ...
                            <td>
                                @can('passenger_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.passengers.show', $passenger->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('passenger_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.passengers.edit', $passenger->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('passenger_delete')
                                    <form action="{{ route('admin.passengers.destroy', $passenger->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('passenger_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.passengers.massDestroy') }}",
