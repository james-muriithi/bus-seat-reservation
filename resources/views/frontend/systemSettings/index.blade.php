        ...
        <div class="col-md-12">
            @can('system_setting_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.system-settings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $systemSetting->id }}">
                                        ...
                                        <td>
                                            @can('system_setting_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.system-settings.show', $systemSetting->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('system_setting_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.system-settings.edit', $systemSetting->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('system_setting_delete')
                                                <form action="{{ route('frontend.system-settings.destroy', $systemSetting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('system_setting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.system-settings.massDestroy') }}",
