        ...
        <div class="col-md-12">
            @can('bus_seat_class_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.bus-seat-classes.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $busSeatClass->id }}">
                                        ...
                                        <td>
                                            @can('bus_seat_class_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.bus-seat-classes.show', $busSeatClass->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('bus_seat_class_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.bus-seat-classes.edit', $busSeatClass->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('bus_seat_class_delete')
                                                <form action="{{ route('frontend.bus-seat-classes.destroy', $busSeatClass->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('bus_seat_class_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.bus-seat-classes.massDestroy') }}",
